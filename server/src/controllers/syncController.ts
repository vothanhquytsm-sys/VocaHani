...
export async function pull(req: AuthenticatedRequest, res: Response) {
  const userId = req.user?.id;
  if (!userId) return res.status(400).json({ error: 'Mã người dùng không hợp lệ.' });

  try {
    // 1. Fetch user progress record
    const progressQuery = db.prepare('SELECT * FROM user_progress WHERE user_id = ?');
    let progress = progressQuery.get(userId) as any;
    ...
    const customWordsQuery = db.prepare('SELECT * FROM user_custom_words WHERE user_id = ?');
    const customWordsRaw = customWordsQuery.all(userId) as any[];
...
export async function push(req: AuthenticatedRequest, res: Response) {
  const userId = req.user?.id;
  if (!userId) return res.status(400).json({ error: 'Mã người dùng không hợp lệ.' });
  ...
  try {
    ...
    const syncTransaction = db.transaction(() => {
      ...
      updateProgress.run(
        userId,
        JSON.stringify(learnedWordIds || []),
        JSON.stringify(favoriteWordIds || []),
      ...
      const deleteCustom = db.prepare('DELETE FROM user_custom_words WHERE user_id = ?');
      ...
      if (customWords && Array.isArray(customWords)) {
        ...
        for (const w of customWords) {
          insertCustom.run(
            ...
            w.topic || 'Từ của tôi',
            w.level || 'A1',
            w.symbolName || 'pencil.circle.fill',
            now
          );
        }
      }
